...
export default function CotationEtudiantsModal({
  ...
  useEffect(() => {
    ...
    const init = {
      cotation: {
        id: param?.cotation?.id ?? "",
        mentionId: param?.mention?.id ?? "",
        semestreId: param?.session?.semestre?.id ?? "",
        anneeId: param?.anneeId ?? "",
        sessionId: param?.session?.id ?? "",
        estCotationAnnuelle: param?.session.estAnnuel ?? false,
      },
      details: []
    };

    inscriptions.forEach((item) => {
      const d = param?.cotation?.details?.find(
        (e) => e?.inscription?.id === item?.id
      );

      init.details.push({
        id: d?.id ?? "",
        cotationId: param?.cotation?.id ?? "",
        inscriptionId: item?.id,
        mentionSemestreEcueId: row?.id ?? "",
        note: d?.note ?? null,
        credit: row?.credit ?? 0,
        estNoteAnnuelle: param?.session.estAnnuel ?? false,
        estTransfere: false,
        estValide: false,
    ...
    setData(init);
  }, [visible, param?.cotation, inscriptions, row, param?.session]);
      ...
      try {
        ...
        showToast(
          `Les cotes des étudiants pour l’ECUE "${row?.ecueName ?? ""}" ont été ${
            param?.cotation?.id ? "modifiées" : "enregistrées"
          } avec succès !`
        );
        ...
        console.error("Erreur save param?.cotation:", error);
        showToast(
            `Erreur lors de ${param?.cotation?.id ? "la modification" : "l'enregistrement"} des cotations`,
            "error"
        );
      } finally {
        setLoading(false);
              ...
              <CRow className="g-2">
                <CCol md={9}>
                  ...
                  <div className="fw-semibold">
                    ...
                    {param?.session?.semestre?.semestreName ?? "-"} |{" "}
                    {param?.anneeId ?? "-"}
                  </div>
                </CCol>
